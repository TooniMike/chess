...
class _GameBoardState extends State<GameBoard> {
  // A2-dimensional list representing the chessboard,
  // with each position containing a chess piece
...
// INITIALIZE BOARD
  void _initializeBoard() {
    ...
    // place random piece in the midele to test
  ...
  List<List<int>> calculateRawValidMoves(int row, int col, ChessPiece? piece) {
    ...
    switch (piece.type) {
      case ChessPieceType.pawn:
        // pawns can move forward it the square is not occupied
  ...
  List<List<int>> calculateRealValidMoves(
    ...
    if (checkSimulation) {
      for (var move in candidateMoves) {
        ...
        // this will simulate the feature move to see if it's safe
  ...
  bool isKingInCheck(bool isWhiteKing) {
    ...
    for (int i = 0; i < 8; i++) {
      for (int j = 0; j < 8; j++) {
        ...
        // check if the king's position is in the piece's valid moves
...
// SIMULATE A FUTURE MOVE TO SEE IF IT'S SAFE (DOSEN'T PUT YOUR OWN KING UNDER ATTACK!)
  bool simulatedMoveIsSafe(
    ...
    // if king is in check = true, means it's not a safe move
    return !kingInCheck;
  }
